...
import { createCustomerProfileDb, getCustomerProfileById, getCustomerProfileByUsername, isLocalDemo, listCustomerProfilesDb, setCustomerUsername, updateCustomerAvatarDb, updateCustomerUsernameDb } from './store.js';
...
export async function customer(request) {
  ...
  try {
    ...
    return {
      ...
      avatarUrl: profile.avatar_url || data.avatarUrl || null,
...
async function localRegister(username, email, password, firstName = '', lastName = '') {
  ...
  const work = localQueue.then(async () => {
    ...
    const user = { id: randomBytes(16).toString('hex'), username, name: fullName, first_name: cleanFirst, last_name: cleanLast, email, salt, hash, avatar_url: null, session_version: 1 };
    users.push(user);
    await mkdir(path.dirname(localFile), { recursive: true });
    await writeFile(localFile, JSON.stringify(users, null, 2));
    return { id: user.id, username, name: fullName, firstName: cleanFirst, lastName: cleanLast, email, avatarUrl: null, version: 1 };
  });
...
async function localLogin(identifier, password) {
  ...
  const names = normalizeUserNames(user);
  return { id: user.id, username: user.username || null, name: names.name, firstName: names.firstName, lastName: names.lastName, email: user.email, avatarUrl: user.avatar_url || null, version: user.session_version || 1 };
}
...
export async function register(username, email, password, firstName = '', lastName = '') {
  ...
  return { user: { id: user.id, email: cleanEmail(user.email), username: profile?.username || username, name: fullName, firstName: cleanFirst, lastName: cleanLast, avatarUrl: profile?.avatar_url || null, version: profile?.session_version || 1 }, confirmationRequired: !result.access_token };
}
export async function login(identifier, password) {
  ...
  return { id: result.user.id, email: cleanEmail(result.user.email), username: profile.username, name: names.name || profile.username || '', firstName: names.firstName, lastName: names.lastName, avatarUrl, version: profile.session_version };
}
...
export async function changeUsername(user, newUsername) {
  ...
  if (isLocalDemo()) {
    const work = localQueue.then(async () => {
      const users = await localRead();
      if (users.some(item => item.id !== user.id && item.username === clean)) {
        throw Object.assign(new Error('Username นี้มีคนใช้แล้ว'), { status: 409 });
      }
      const found = users.find(item => item.id === user.id);
      if (!found) throw Object.assign(new Error('ไม่พบบัญชีผู้ใช้'), { status: 404 });
      found.username = clean;
      await writeFile(localFile, JSON.stringify(users, null, 2));
  ...
  const existing = await getCustomerProfileByUsername(clean);
  if (existing && existing.user_id !== user.id) {
    throw Object.assign(new Error('Username นี้มีคนใช้แล้ว'), { status: 409 });
  }
  await updateCustomerUsernameDb(user.id, clean);
  return { ...user, username: clean };
}

export async function updateCustomerAvatar(user, avatarUrl) {
  if (isLocalDemo()) {
    const work = localQueue.then(async () => {
      const users = await localRead();
      const found = users.find(item => item.id === user.id);
  ...
  await updateCustomerAvatarDb(user.id, avatarUrl);
  const url = process.env.SUPABASE_URL?.replace(/\/$/, '');
  const apikey = process.env.SUPABASE_SECRET_KEY;
  if (url && apikey) {
    try {
      await fetch(`${url}/auth/v1/admin/users/${user.id}`, {
        method: 'PUT',
        headers: { apikey, Authorization: `Bearer ${apikey}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ user_metadata: { avatar_url: avatarUrl, picture: avatarUrl } })
      });
    } catch {}
  }
  return { ...user, avatarUrl };
...
export function getGoogleAuthUrl(origin) {
  if (isLocalDemo()) {
    return `${origin}/#demo_google_login`;
  }
  const url = process.env.SUPABASE_URL?.replace(/\/$/, '');
  if (!url) throw Object.assign(new Error('ยังไม่ได้ตั้งค่า Supabase'), { status: 503 });
  const redirectUri = encodeURIComponent(`${origin}/`);
  return `${url}/auth/v1/authorize?provider=google&redirect_to=${redirectUri}`;
}

export async function verifyGoogleToken(accessToken) {
  if (isLocalDemo() || (typeof accessToken === 'string' && accessToken.startsWith('demo-google-token'))) {
    const isCustom = accessToken !== 'demo-google-token';
    const tag = isCustom ? accessToken.replace(/[^a-zA-Z0-9]/g, '_').slice(-16) : 'default';
    const email = isCustom ? `${tag}@polyloot.store` : 'demo.google@polyloot.store';
    const baseUsername = isCustom ? `poly_${tag}`.slice(0, 24) : 'poly_gamer';
    const demoUser = {
      id: isCustom ? `demo-google-${tag}` : 'demo-google-user-01',
      email,
      first_name: 'Poly',
      last_name: 'Gamer',
      name: 'Poly Gamer',
      username: baseUsername,
      avatar_url: 'https://api.dicebear.com/7.x/bottts/svg?seed=PolyGamer',
      salt: '',
      hash: '',
      session_version: 1
    };
    const work = localQueue.then(async () => {
      const users = await localRead();
      let found = users.find(u => u.email === demoUser.email);
      if (!found) {
        users.push(demoUser);
        found = demoUser;
        await mkdir(path.dirname(localFile), { recursive: true });
      ...
      return {
        ...
        version: found.session_version || 1
      };
    });
    localQueue = work.catch(() => {});
    return work;
  ...
  if (!userRes.ok) {
    throw Object.assign(new Error('การยืนยันตัวตนด้วย Google ไม่ถูกต้องหรือหมดอายุ'), { status: 401 });
  }

  const userData = await userRes.json();
  if (!userData?.id || !userData?.email) {
    throw Object.assign(new Error('ไม่พบข้อมูลผู้ใช้จาก Google'), { status: 401 });
  }

  let profile = await getCustomerProfileById(userData.id);
  const meta = userData.user_metadata || {};
  const names = normalizeUserNames({
    ...meta,
    first_name: meta.given_name || meta.first_name,
  ...
  if (!profile) {
    const baseUsername = (meta.user_name || meta.preferred_username || userData.email.split('@')[0] || 'user')
      .toLowerCase().replace(/[^a-z0-9_]/g, '_').slice(0, 20);
    let resolvedUsername = baseUsername || 'user';
    if (await getCustomerProfileByUsername(resolvedUsername)) {
      resolvedUsername = `${resolvedUsername.slice(0, 18)}_${Math.floor(100 + Math.random() * 900)}`;
    }
    try {
      await createCustomerProfileDb({
        user_id: userData.id,
      ...
      profile = await getCustomerProfileById(userData.id);
    } catch {}
  } else if (!profile.avatar_url && googleAvatar) {
    try {
      await updateCustomerAvatarDb(userData.id, googleAvatar);
  ...
  return {
    ...
    email: cleanEmail(userData.email),
    username: profile?.username || null,
    name: names.name || profile?.username || '',
    firstName: names.firstName,
    lastName: names.lastName,
    avatarUrl: profile?.avatar_url || googleAvatar || null,
    version: profile?.session_version || 1
  };
}
...
export async function listCustomerProfiles() {
  ...
  return profiles.map(p => ({ ...p, avatar_url: p.avatar_url || null, name: p.username || '', firstName: '', lastName: '', first_name: '', last_name: '' }));
}
