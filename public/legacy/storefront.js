...
function productCard(item) { return `<article class="product-card"><a class="product-cover" href="#asset/${esc(item.id)}" aria-label="ดูรายละเอียด ${esc(item.title)}">${cover(item)}</a><div class="product-copy"><span class="product-category">${esc(item.category || '3D Asset')}</span><h3><a href="#asset/${esc(item.id)}">${esc(item.title)}</a></h3><p class="product-description">${esc(item.description)}</p><div class="product-specs"><span>${esc((item.formats || []).slice(0, 2).join(' / ') || '3D')}</span><span>${esc(item.license || 'ดูสิทธิ์การใช้งาน')}</span></div><div class="product-bottom"><strong>${money(item.price)} <small>ราคาเดโม</small></strong><button class="pill-button dark" type="button" data-add="${esc(item.id)}">เพิ่มลงตะกร้า</button></div></div></article>`; }
...
function home() {
  ...
  setView(`
    <section class="home-hero"><div class="home-hero-copy"><span class="home-eyebrow">POLYLOOT / 3D GAME ASSETS</span><h1>หาแอสเซ็ตที่ใช่<br>แล้วสร้างเกมของคุณ</h1><p>โมเดล 3D สำหรับนักพัฒนาเกม เลือกดูรายละเอียดไฟล์ ทดลองสั่งซื้อ และรับไฟล์ในคลังของคุณ</p><div class="hero-actions"><a class="pill-button dark" href="#catalog">เลือกดูสินค้า <span aria-hidden="true">↗</span></a><a class="hero-text-link" href="#library">ไปที่คลังของฉัน →</a></div><p class="hero-disclaimer">ร้านสาธิตเพื่อการศึกษา · ราคาจำลอง · ไฟล์ต้นฉบับจาก Kenney</p></div><div class="home-hero-art"><img src="/assets/previews/blocky-characters.png" alt="ตัวอย่างชุด Blocky Characters จาก Kenney"></div></section>
